...
public class Linklist4 {
	...
	public int GetIDCutvalue(int b,int c,Connection con)throws SQLException{
		...
		int i;
		int t=0;
		/*sql="select * from tree4 where 属性="+b+" and 取值="+c;
		...
		for(i=1;i<25;i++){
			...
			sql="select * from tree4 where 属性="+b+" and 取值="+(c+i);
			rs = stmt.executeQuery(sql);
			if(rs.next())
			{
				t=rs.getInt(6);
	...
	public void Creatlistlink(int b,int c,int t,Connection con,Linklist list[],int idnum[])throws SQLException
	{
		...
		IDcut=min(idnum);
		String sql="select * from tree4 where 属性="+b+" and 取值="+c+" and ID<="+IDcut;
		rs = stmt.executeQuery(sql);
	...
	public  double[]  Linklistmain(double leafrate[]) throws SQLException {
		...
		for(int i=0;i<len;i++){
			System.out.print("决策树4"+leafrate[i]+",");}
		return  leafrate;
	}
}
