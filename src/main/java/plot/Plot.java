...
//	static ArrayList<OWLIndividual> EventIndividual=new ArrayList<OWLIndividual>();

	//程序入口
	public static  Document EventInfer(ArrayList<String> topiclist,ArrayList<String> actionTemplateAttr, OWLModel owlModel,
									   String maName,Document doc) {
	...
	//20170616
	public static Document setIndividual(ArrayList<String> emotiontopic,ArrayList<String>noemotiontopic,OWLModel model,String maName,Document doc)
	{
		int count=0;
		...
		if(AllAddPeopleIndividuals.size()!=0)
		{
			if(noemotiontopic.size()!=0)
			{
				for(int ins=0;ins<noemotiontopic.size();ins++)
				{
					...
					for(Iterator is=AllAddPeopleIndividuals.iterator();is.hasNext();)
					{
						...
						//20171108
						String  PeopleNameInOwl=((OWLIndividual)owlindi.getPropertyValue(hasModelNameProperty)).getBrowserText();
						int pos = PeopleNameInOwl.indexOf(":");
						String PeopleName =(String)PeopleNameInOwl.subSequence(pos+1, PeopleNameInOwl.length());
						if(!PeopleName.equals("M_bikeboy.ma")&&ob.toString().equals("people"))
						{
					...
					if(fage==false)
					{
						...
						for(int ins1=0;ins1<emotiontopic.size();ins1++)
						{
							for(Iterator is=AllAddPeopleIndividuals.iterator();is.hasNext();)
							{
								...
								if(!PeopleName.equals("M_bikeboy.ma")&&ob.toString().equals("people"))
								{
									if(owlindi.getPropertyValueCount(topicname)!=0)
									{
										if(owlindi.getPropertyValue(topicname).toString().equals(emotiontopic.get(ins1)))
										{
											fl=true;
						...
						if(fl==false)
						{
							...
							doc=WriteXML1(model,doc,str,ary.get(l).getBrowserText().toString(),father,spacename,"people","1",noemotiontopic.get(ins));
						}
					}
				}
			}
				...
				for(int ine=0;ine<emotiontopic.size();ine++)
				{
					...
					for(Iterator is1=AllAddPeopleIndividuals.iterator();is1.hasNext();)
					{
						...
						//20171108
						String  PeopleNameInOwl=((OWLIndividual)owlindi1.getPropertyValue(hasModelNameProperty)).getBrowserText();
						int pos = PeopleNameInOwl.indexOf(":");
						String PeopleName =(String)PeopleNameInOwl.subSequence(pos+1, PeopleNameInOwl.length());
						if(!PeopleName.equals("M_bikeboy.ma")&&ob1.toString().equals("people"))
						{
					...
					if(fage1==false)
					{
						...
						doc=WriteXML1(model,doc,stri,aryy.get(m).getBrowserText().toString(),father1,spacename1,"people","1",emotiontopic.get(ine));
					}
			...
			if(noemotiontopic.size()!=0){
				for(int ine=0;ine<noemotiontopic.size();ine++)
				{
					...
					doc=WriteXML1(model,doc,stri,aryy.get(m).getBrowserText().toString(),father1,spacename1,"people","1",noemotiontopic.get(ine));
				}
			}
	...
	private static ArrayList<OWLNamedClass> getEvent1(String noemotiontopic1,
	...
	/*	if(NoEmotionEvent1.size()==0)
			flage.add("false");//判断是否还有主题   flage=false表示没有与之对应的事件
		else
			flage.add("true");*/
	...
	//得到与主题相关的事件类
	public static ArrayList<OWLNamedClass> getEvent(ArrayList<String> emotiontopic,OWLObjectProperty eventop,OWLModel owlModel)
	{
	...
	public static ArrayList<OWLNamedClass> getEventFromTemplate(ArrayList<String> actiontemplate,OWLObjectProperty eventop,OWLModel owlModel)
	{
		ArrayList<OWLNamedClass> emotione=new ArrayList<OWLNamedClass>();
		ArrayList<String> topicclass=new ArrayList<String>();
		ArrayList<String> emotionclass=new ArrayList<String>();
	...
	public static ArrayList<OWLNamedClass> getActionEvent(ArrayList<String> actiontemp,OWLObjectProperty eventop,OWLModel owlModel)
	{
	...
	public static ArrayList<OWLNamedClass> isOppositeofSameTopic(OWLModel model,ArrayList<String>emotionclass,ArrayList<String>emotiontopic)
	{
	...
	//判断添加的模型中是否含有人物
	public static Document getPeople(OWLModel model,String maName,String noemotiontopic1,ArrayList<OWLNamedClass>noEmotionEvent,boolean f,Document doc)
	{
		...
		for(Iterator it=AllAddPeopleIndividuals.iterator();it.hasNext();)
		{
			...
			int pos = PeopleNameInOwl.indexOf(":");
			String PeopleName =(String)PeopleNameInOwl.subSequence(pos+1, PeopleNameInOwl.length());
			if(!PeopleName.equals("M_bikeboy.ma")&&addmodlins.equals("people") && noemotiontopic1.equals(addmodlin) && addmodlIsDeal.equals("false"))
			{//添加的模型的主题与该主题一致且是人物
			...
			}else if(!PeopleName.equals("M_bikeboy.ma")&&addmodlins.equals("people")&& addmodlIsDeal.equals("false")){
	...
	public static Document getPeopleToTemplate(OWLModel model,String maName,ArrayList<OWLNamedClass>actionEvent,Document doc)
	{
		...
		for(Iterator it=AllAddPeopleIndividuals.iterator();it.hasNext();)
		{
			...
			int pos = PeopleNameInOwl.indexOf(":");
			String PeopleName =(String)PeopleNameInOwl.subSequence(pos+1, PeopleNameInOwl.length());
			if(!PeopleName.equals("M_bikeboy.ma")&&addmodlins.equals("people")){
			...
			//0530
			//20171108
			if(!PeopleName.equals("M_bikeboy.ma")&&addmodlins.equals("people")&&addmodeIsUsed.equals("false"))
			{//添加的模型的主题与该主题一直且是人物
				//	if(addModelIndiviual.getPropertyValueCount(isSuited)==0)
				//	{
	...
	public static Document print(ArrayList<OWLIndividual> EventIndividual,OWLModel model,Document doc)
	{
		...
		for(Iterator it=EventIndividual.iterator();it.hasNext();)
		{
			...
			//	OWLIndividual SL=(OWLIndividual)ind.getPropertyValue(hasReference);

			String reference="";
	/*		if(SL!=null)
				reference=SL.getBrowserText().toString();
			else
				reference="";*/
	...
	//读取xml
	public static Document WriteXML(OWLModel model,Document doc,String event,String id,String people,String action, String distence,String reference,String f)
	{
	...
	public static Document WriteXML1(OWLModel model,Document doc,String id,String maname,String father, String spacename,String type,String number,String topicname)
	{
		Element rootName = (Element)doc.getRootElement();
		Element name = rootName.element("maName");
		Element ruleName = name.addElement("rule");
	...
	//获得与主事件相关的事件
	public static ArrayList<OWLIndividual> getLinkEvent(ArrayList<OWLNamedClass> EmotionEvent,ArrayList<OWLNamedClass> NoEmotionEvent,OWLModel model,Document doc)
	{
	...
	public static ArrayList<OWLIndividual> getLinkEvent1(OWLModel model,Document doc,ArrayList<OWLNamedClass>emotionEvent)
	{
		...
		if(emotionEvent.size()!=0)
		{
			if(emotionEvent.size()>1)
			{//如果情绪事件个数大于一，则从最后一个情绪事件中选择它的effect事件，这样组成多个事件组合
	...
	public static ArrayList<OWLIndividual> getIndividual(OWLNamedClass mainEvent,ArrayList<OWLNamedClass> Event,OWLModel model,Document doc)//获取事件实例，并添加动作和参照物
	{
	...
	/*	else{
			for(int j=0;j<Event.size();j++){
				String event=Event.get(j).getBrowserText().toString();
				if(event.equals(str+"WalkEvent") || event.equals(str+"RunEvent")){
					ArrayList addModel=new ArrayList();//存放于主要事件对应的主题
					RDFResource topic= mainEvent.getSomeValuesFrom(hasSuitTopic);
					if(topic!=null)
					{
						String topicstring=topic.getBrowserText();
						if(topicstring.contains("or"))
						{
							String[] hasvalue=topicstring.split("or");
							for(int m=0;m<hasvalue.length;m++)
							{
								addModel.add(hasvalue[m]);
							}
						}
						else
							addModel.add(topicstring);
					}
					Collection clos=AddModelRelatedClass.getInstances();
					ArrayList<OWLIndividual> ground=new ArrayList<OWLIndividual>();
					loop:for(Iterator is=clos.iterator();is.hasNext();)
					{
						OWLIndividual owls=(OWLIndividual) is.next();
						OWLIndividual spac=(OWLIndividual) owls.getPropertyValue(addModelRelatedSpace);
						if(spac!=null)
						{

							OWLNamedClass spacePlace=(OWLNamedClass) spac.getDirectType();
							String topicli=(String) owls.getPropertyValue(topicname);

							for(int k=0;k<addModel.size();k++)
							{
								String s=addModel.get(k).toString();
								if(s.equals(topicli))
								{
									if(spacePlace.getBrowserText().contains("Ground") && owls.getPropertyValue(addModelTypeProperty).equals("model")
											&& !owls.getPropertyValue(hasModelNameProperty).equals("M_floor.ma"))
										reference=(OWLIndividual) owls.getPropertyValue(hasModelNameProperty);
									break loop;
								}
							}
							if(spacePlace.getBrowserText().contains("Ground") && owls.getPropertyValue(addModelTypeProperty).equals("model")
					...
					if(ground.size()!=0 && reference==null)
					{
						int k=random(ground.size());
						OWLIndividual re=ground.get(k);
						reference=(OWLIndividual) re.getPropertyValue(hasModelNameProperty);
					}
					if(reference==null)
					{
						reference=setReference(model);
	...
	//判断是否有人物没有主题
	public static Document addActionToModel(OWLModel model,Document doc,ArrayList<String>topiclist2,ArrayList list)
	{
		OWLNamedClass AddModelRelatedClass =model.getOWLNamedClass("AddModelRelated");
		OWLDatatypeProperty addModelTypeProperty=model.getOWLDatatypeProperty("addModelType");
		OWLDatatypeProperty modelIDProperty=model.getOWLDatatypeProperty("modelID");
		...
		if(clo.size()!=0)
		{
			for(Iterator i=clo.iterator();i.hasNext();)
			{
				...
				//20171108
				String  PeopleNameInOwl=((OWLIndividual)owli.getPropertyValue(hasModelNameProperty)).getBrowserText();
				int pos11 = PeopleNameInOwl.indexOf(":");
				String PeopleName =(String)PeopleNameInOwl.subSequence(pos11+1, PeopleNameInOwl.length());
				if(!PeopleName.equals("M_bikeboy.ma")&&owli.getPropertyValue(addModelTypeProperty).equals("people"))
					...
					if(isUsed.equals("false")){
						...
						if(flage2==true)
						{
							...
							for (Iterator itTopic = topiclist1.iterator(); itTopic.hasNext();)
							{
								...
								OWLObjectProperty actionSuitableForTopicProperty = model.getOWLObjectProperty("p2:actionSuitableForTopic");
							...
							else
							{
								...
								if (templateList.length > 0) {
									for (int i1 = 0; i1 < templateList.length; i1++) {
										...
										Collection mapToActionValues = actTemIndividual.getPropertyValues(mapToActionProperty);
							...
							if(ifAddConstraint.equals("1")){
								OWLDatatypeProperty constraintTypeProperty=model.getOWLDatatypeProperty("p2:actionConstraintType");
								...
								OWLObjectProperty relativeModelProperty=model.getOWLObjectProperty("p2:modelSuitableForAction");
								...
								OWLDatatypeProperty modelRelativePositionProperty=model.getOWLDatatypeProperty("p2:actionRelativeModelPosition");
							...
							flage.add("true");
							doc=WriteXML(model,doc,str1,id,modelname,finalActionName,"default","null",ifAddConstraint);
	 ...
	 */
	//20180326YY
	public static Document setIndividualToInteraction(ArrayList<String> emotiontopic,ArrayList<String>noemotiontopic,OWLModel model,String maName,Document doc)
	{
		int count=0;
		...
		if(AllAddPeopleIndividuals.size()!=0)
		{
			...
			if(fage==false)
			{
				...
				if(fl==false)
				{
					...
					addModelIndiviual.addPropertyValue(addModelTypeProperty, "people");
					addModelIndiviual.addPropertyValue(isSuited, "false");
					//addModelIndiviual.addPropertyValue(topicname, noemotiontopic.get(0));
					ArrayList<OWLIndividual> ary=new ArrayList<OWLIndividual>();
					ary.add(model.getOWLIndividual("M_boy.ma"));
					ary.add(model.getOWLIndividual("M_girl.ma"));
					int l=random(ary.size());
					...
					doc=WriteXML1(model,doc,str,ary.get(l).getBrowserText().toString(),father,spacename,"people","1",null);
				}
			}
