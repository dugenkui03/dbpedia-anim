...
public class sceneselectbyDecisionTree {
	...
	public  double[] tmain (String englishTopicStr,ArrayList<String> englishTemplate,ArrayList<SceneCase> sceneList,int scenecount,double leafrate[])throws SQLException{
	   ...
	   /* String s="";
		...
		leafrate1=list.Linklistmain(leafrate);

		Linklist2 list2=new Linklist2();
		leafrate2=list2.Linklistmain(leafrate);

		Linklist3 list3=new Linklist3();
		leafrate3=list3.Linklistmain(leafrate);

		Linklist4 list4=new Linklist4();
		leafrate4=list4.Linklistmain(leafrate);



		for(int i=0;i<scenecount;i++){
			System.out.println(leafrate1[i]+"哈哈"+leafrate2[i]+"哈哈"+leafrate3[i]+"哈哈"+leafrate4[i]+"\r");
			leafrate[i]=(leafrate1[i]+leafrate2[i]+leafrate3[i]+leafrate4[i])/4;
		}
